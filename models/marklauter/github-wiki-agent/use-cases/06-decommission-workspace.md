...
- **Supporting actors:** actors/17-git (working tree inspection)
...
No agents are involved. The actors/01-user drives the interaction directly through the `/down` command, which coordinates all steps. This is a human-driven workflow, not an agent-orchestrated one.

- **actors/17-git** — Inspects the wiki working tree for uncommitted changes and unpushed commits. Produces the safety report consumed by the system before removal proceeds.

## Invariants

See also: invariants/index

- **Decommissioning never provisions.** `/down` will not create, modify, or initialize any workspace state. Provisioning is exclusively the concern of 05-provision-workspace.
...
1. **actors/01-user** — Initiates decommissioning by running `/down`.
...
4. **actors/17-git** — Checks the wiki working tree for uncommitted changes and unpushed commits.
5. **System** — Removes all workspace artifacts.
   --> WorkspaceDecommissioned
6. **actors/01-user** — Sees confirmation of what was removed: repo identity, directories deleted, config file deleted, and remaining workspace count. If other workspaces remain, lists them. If none remain, notes that `/up` can provision a new one.
...
3. **actors/01-user** — Either types the repo name to confirm, or cancels.
...
- **events/07-workspace-decommissioned** — Workspace removed; no longer discoverable.
...
- **invariants/08-commands-do-not-chain** When unsaved work is detected, the system tells the user to publish their work using git (commit and push) but does not do it for them. The user cancels `/down`, publishes independently using whatever git tool they prefer, and then re-runs `/down`.
- **Source clone has no safety concern.** The source repo is readonly (invariants/03-source-repo-readonly invariant from 05-provision-workspace). It was never mutated, so it can always be deleted without data loss. Only the wiki clone requires a safety check.
- **invariants/07-scripts-own-deterministic-behavior** The safety check (`check-wiki-safety.sh`) and removal (`remove-workspace.sh`) are separate scripts by design. The safety check produces a report; the removal script acts on it. The `/down` command delegates to both scripts rather than inlining git commands.

...
2. **"Consolidation is a one-time cleanup phase."** No. Consolidation happens continuously. The first time an invariant appears in two use cases, you extract it to SHARED-INVARIANTS.md. The first time an actor appears under different names, you settle the name in ACTOR-CATALOG.md. Consolidation in design-cycle.md is a tendency, not a gate. Backtracking is the process, not an exception to it.
